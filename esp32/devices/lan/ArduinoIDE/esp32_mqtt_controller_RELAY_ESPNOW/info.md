...
This is the separate ESP-NOW bridge copy. Start with
[the bridge guide](../../../../espnow/README.md), which overrides the legacy
transport description below. Bridge mode uses LAN MQTT with same-channel WiFi
fallback enabled and fixed-channel ESP-NOW. Configure peers/routes in `config.cpp` and the
ignored `espnow_local.h`. The original sibling sketch is the rollback backup.
